@props(['types' => [], 'action' => 'favorite'])

@foreach ($types as $dishType => $dishes)
    @foreach ($dishes as $dish)
        <div class="flex justify-between rounded-lg bg-[rgba(255,255,255,0.3)] p-3 shadow-sm gap-3">
            ...
            <form action="{{ route($action) }}" method="POST" class="self-center">
                ...
                <button type="submit" class="text-nowrap bg-gray-50 shadow-sm px-2 py-1 rounded hover:bg-red-50 cursor-pointer">{{ $slot }}</button>
            </form>
        </div>
    @endforeach
@endforeach